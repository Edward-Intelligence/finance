...
module Finance
  ...
  module Cashflow
    ...
    def xirr(guess = nil)
      ...
      rate = [valid(guess)]
      nlsolve(func, rate)
      Rate.new(rate[0], :apr, compounds: Finance.periodic_compound ? :continuously : :annually)
    end
    ...
    def days_in_period
      if Finance.periodic_compound && Finance.business_days
    ...
    def valid(guess)
      if guess.nil?
        raise ArgumentError, 'Invalid Guess. Default guess should be a [Numeric] value.' unless Finance.guess.is_a? Numeric
        Finance.guess
      else
